...
if ! (oc get istag/amq-broker-72-openshift:${AMQ_BROKER_REL} -n openshift &>/dev/null); then
    oc import-image amq-broker-7/amq-broker-72-openshift:${AMQ_BROKER_REL} --from=registry.access.redhat.com/amq-broker-7/amq-broker-72-openshift --confirm -n openshift
fi
