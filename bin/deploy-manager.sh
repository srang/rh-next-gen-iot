...
if (oc get deploy/${APPLICATION_NAME} -n ${NAMESPACE}); then
    oc rollout pause dc/${APPLICATION_NAME} -n ${NAMESPACE} || echo "already paused"
fi

oc process -f ${PROJ_DIR}/${APPLICATION_CONTEXT_DIR}/templates/lab-manager.yml \
